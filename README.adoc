...
When a `@Component` parameter uses generic type arguments, Pixie will narrow matching to only those
components whose resolved generic types are compatible. This works for interfaces, superclasses, and
any depth of inheritance.
...
public class ApiGateway {
    public ApiGateway(
            ...
            final RequestHandler<APIGatewayProxyRequestEvent, APIGatewayV2HTTPResponse> handler) {
        // Only RequestHandler implementations with <APIGatewayProxyRequestEvent, APIGatewayV2HTTPResponse>
        // will be injected. Other RequestHandler implementations are ignored.
    }
}
----
...
==== Collection Injection with Generics

Generic matching also applies to collection injection points. Only components with matching type arguments
are collected:

[source,java]
----
// Collects only RequestHandler<String, Integer> implementations — not all RequestHandler impls
@Param("handlers") @Component List<RequestHandler<String, Integer>> handlers
----

==== Producer Type Support
...
Mixed scenarios where some type arguments come from the producer declaration and others from the class
hierarchy are fully supported. For example, a factory returning `BooleanHandler<String>` where
`BooleanHandler<I> implements RequestHandler<I, Boolean>` will correctly resolve to `RequestHandler<String, Boolean>`.
